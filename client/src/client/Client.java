...
public class Client {
        ...
        try (SocketChannel socketChannel = SocketChannel.open();
            ...
            while (true) {
                ...
                try {
                    ...
                    System.out.println(ColorConsole.RED + "При создании запроса произошла ошибка: " + e.getMessage() + ColorConsole.RESET);
                    continue;
                }
                for (Request request : requests) {
                    ...
                    System.out.println(ColorConsole.YELLOW + "Отправка: " + request + ColorConsole.RESET);
    ...
    private static void sendRequest(SocketChannel channel, Request request)
            throws IOException {
    ...
    private static void printResponse(Response response, String commandName) {
        if (response.isSuccess()) {
            System.out.println(ColorConsole.GREEN + "Результат выполнения команды " + commandName + ":\n" + ColorConsole.RESET + response.getMessage());
            historyCommand.addInHistory(currentCommand);
        } else {
            System.out.println(ColorConsole.RED + "При выполнении команды " + commandName +" возникла ошибка: " + response.getMessage() + ColorConsole.RESET);
        }
    }
}